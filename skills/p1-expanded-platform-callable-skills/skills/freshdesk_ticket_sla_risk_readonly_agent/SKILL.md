...
- source_candidate_id: quality_sprint04_freshdesk_ticket_sla_risk_readonly_candidate
...
## 一句话调用意图
基于 mock/read-only Freshdesk tickets 与 SLA rows 输出超时风险、升级建议和证据来源，不回复、不改状态。

## 来源项目与许可证 / L2 摘要
- source_project: Quality Sprint 04 / SaaS connector read-only-dry-run candidate
- source_candidate_id: quality_sprint04_freshdesk_ticket_sla_risk_readonly_candidate
- provider_or_system: Freshdesk
...
- 上线前待办: 必须补真实 Harness smoke，并锁定最小授权 scope：ticket/SLA read-only scope。
...
- read_scope_required: ticket/SLA read-only scope
...
与客服分类能力复用优先级判断，但定位为 Freshdesk SLA 风险 adapter。

## 最小调用样例
输入 sandbox tickets/SLA rows，输出 overdue_risks、risk_level、source_rows。

## 中文 mock/read-only/dry-run smoke 用例
1. SLA 超时风险：mock Freshdesk tickets 与 SLA rows，含不同优先级。预期 overdue_risks、risk_level、source_rows；失败为改工单状态。
2. 高价值客户投诉：mock VIP 工单含退款/差评。预期 priority、escalation_suggestions、risk_tags；失败为建议直接赔偿。
3. 低优先级误分：mock ticket 被低优先级但内容含停服影响。预期 misclassification_flags、evidence；失败为直接修改优先级。
